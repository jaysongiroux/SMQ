...
| `buffer_flush_interval_ms` | integer | Interval in milliseconds between buffer flushes to the database. Lower values reduce potential data loss window but increase write frequency and database load. Balance based on durability vs. performance requirements. **Note:** With adaptive flushing enabled, this serves as the base interval; actual batch sizes adjust dynamically. | **Yes**     | Min: 1000                         | -       |
| `buffer_max_size_kb`       | integer | Maximum size in kilobytes for messages to batch before forcing a flush. When reached, triggers immediate flush regardless of `buffer_flush_interval_ms`. **Note:** This serves as the starting batch size when adaptive flushing is disabled, or the initial size when adaptive is enabled.                                                  | **Yes**     | 10-1000000                        | -       |
...
Adaptive flushing automatically adjusts batch sizes based on real-time database performance and message throughput. This prevents flush overlap (when flushes take longer than the flush interval) and optimizes throughput by scaling batch sizes dynamically.

| Field / Variable                 | Type    | Description                                                                                                                                                                                                                                                                                                   | Required    | Validation                                                                                  | Default |
| -------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------- | ------- |
| `buffer_adaptive`                | boolean | Enable adaptive batch sizing. When enabled, the buffer automatically adjusts batch size based on flush performance. **Recommended for production**, especially in multi-region deployments or when database performance varies. When disabled, uses static `buffer_max_size_kb`.                              | No          | `true` or `false`                                                                           | `false` |
| `buffer_adaptive_max_size`       | integer | Maximum batch size (in number of messages) when adaptive flushing is enabled. The buffer will scale up to this ceiling during high throughput with fast database performance. **Must be larger than `buffer_max_size_kb`**. Typical values: 5000-10000 for high throughput, 2000-5000 for balanced workloads. | Conditional | 100-100000 (required if `buffer_adaptive=true`)                                             | -       |
| `buffer_adaptive_min_size`       | integer | Minimum batch size (in number of messages) when adaptive flushing is enabled. The buffer will not reduce batch size below this floor even during low throughput or slow database scenarios. Prevents excessive small flushes. Typical values: 50-200.                                                         | Conditional | 10-10000 (required if `buffer_adaptive=true`, must be less than `buffer_adaptive_max_size`) | -       |
| `buffer_adaptive_tune_threshold` | integer | Number of flushes to complete before adjusting batch size. Lower values (2-3) = more responsive but may oscillate. Higher values (5-10) = more stable but slower to adapt. **Recommended:** 5 for most workloads, 3 for rapidly changing traffic, 8-10 for steady-state workloads.                            | Conditional | 1-20 (required if `buffer_adaptive=true`)                                                   | `5`     |
